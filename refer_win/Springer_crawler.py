...
# =========================
# Windows: 在脚本里内置三处关键路径（可直接改成本机绝对路径）
# =========================
IS_WINDOWS = (platform.system().lower() == 'windows')

DEFAULT_LIBREOFFICE_PATH = r"...\\soffice.exe"
DEFAULT_UNO_PROGRAM_DIR  = r"C:...\\LibreOffice\\program"      # 用于 UNO_PATH
DEFAULT_UNOCONV_PATH     = r"C:...\\LibreOffice\\unoconv-master\\unoconv.py"  # 也可设为 unoconv.exe
DEFAULT_CHROMEDRIVER     = r"C:...\\chromedriver.exe"

# 你也可以选择把上面四个常量改成从环境变量读取（可选）
LIBREOFFICE_PATH = os.environ.get("LIBREOFFICE_PATH", DEFAULT_LIBREOFFICE_PATH)
UNO_PROGRAM_DIR  = os.environ.get("UNO_PROGRAM_DIR",  DEFAULT_UNO_PROGRAM_DIR)
UNOCONV_PATH     = os.environ.get("UNOCONV_PATH",     DEFAULT_UNOCONV_PATH)
CHROMEDRIVER_PATH= os.environ.get("CHROMEDRIVER_PATH",DEFAULT_CHROMEDRIVER)

def start_libreoffice():
    """启动 LibreOffice（headless，开启 UNO socket）——严格 def 形式"""
    if not IS_WINDOWS:
        return
    try:
        subprocess.Popen([
        ...
        print(f"启动 LibreOffice 失败: {e}")

def convert_to_pdf_with_unoconv(file_path: str) -> Optional[str]:
    """使用 unoconv 将 .doc/.docx 转为 PDF —— 严格 def 形式"""
    ext = os.path.splitext(file_path)[1].lower()
    if ext not in ('.doc', '.docx'):
        return None

    if IS_WINDOWS:
        # 设置 UNO_PATH 并确保 LO 已经 headless 运行
        os.environ['UNO_PATH'] = UNO_PROGRAM_DIR
        start_libreoffice()
        time.sleep(2)  # 稍等一会儿

    out_pdf = os.path.splitext(file_path)[0] + '.pdf'
    try:
        # 兼容 .py 与可执行
        if UNOCONV_PATH.lower().endswith('.py'):
            subprocess.run([sys.executable, UNOCONV_PATH, '-f', 'pdf', file_path], check=True)
        else:
            subprocess.run([UNOCONV_PATH, '-f', 'pdf', file_path], check=True)

        if os.path.exists(out_pdf):
            print(f"已将 {file_path} 转换为 PDF")
            return out_pdf
        else:
            print("unoconv 运行后未找到输出 PDF。")
            return None
    except subprocess.CalledProcessError as e:
        print(f"转换 {file_path} 为 PDF 时出错: {e}")
        return None
    except Exception as e:
        print(f"unoconv 执行异常: {e}")
        return None
...
class PDFProcessor:
    ...
    def setup_logging(self):
        ...
        console_handler.setFormatter(
            logging.Formatter('%(asctime)s - %(levellevel)s - %(message)s'.replace('levellevel','levelname'))
        )
...
article_urls = df.iloc[:, 12].tolist() # article_url
...
for code, doi, url in zip(ccdc_codes, article_dois, article_urls):
    ...
    try:
        ...
        response = requests.get(url_api, headers={"User-Agent": user_agent})
        requests_made += 1
        ...
        if response.status_code == 200:
            ...
            if pdf_url:
                # 下载正文 PDF
                pdf_response = requests.get(pdf_url, stream=True, headers={"User-Agent": user_agent})
        ...
        # —— 指定 ChromeDriver 的路径（严格使用 Service + 绝对路径）——
        if IS_WINDOWS:
                    ...
                    for link in links:
                        ...
                        if low.endswith((".pdf", ".docx", ".doc")):
                            ...
                            if file_response.status_code == 200:
                                ...
                                print(f"下载失败（{filename}），状态码：{file_response.status_code}")
            browser.quit()
        except Exception as e:
            try:
                browser.quit()
        ...
        for filename in os.listdir(article_dir):
            ...
            if filename.lower().endswith(('.doc', '.docx')):
                converted_pdf = convert_to_pdf_with_unoconv(file_path)
